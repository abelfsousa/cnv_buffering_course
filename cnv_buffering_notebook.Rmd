---
title: "Analysis of CNV buffering at the protein level"
...
### simple barplot with the number of samples by cancer type
...
df must a data.frame or tibble

x and y the variables to select and correlate from df

uses pearson correlation by default

the output is a table with two columns, one with the correlation and other with the respective P-value
```{r}
...
### calculate the correlations between the CNV and the RNA and the CNV and the protein
```{r}
...
```{r, fig.height = 4, fig.width = 4}
# let's filter the multi-omics data by selecting the PDCD5 gene
attenuation_example2 <- multi_omics %>%
  filter(gene == "PDCD5")
attenuation_example2

# CNV vs RNA
cnv_rna_example2 <- attenuation_example2 %>%
  ggplot(mapping = aes(x = cnv, y = rna, group = cnv, fill = as.character(cnv))) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.1, size = 0.5) +
  scale_fill_discrete(guide = F) +
...
# in case you want to save the plot to a pdf file uncomment the following line
#ggsave(filename = file.path(".", "cnv_rna_example2.pdf"), plot = cnv_rna_example2, height = 4, width = 4)


# CNV vs protein
cnv_protein_example2 <- attenuation_example2 %>%
  ggplot(mapping = aes(x = cnv, y = protein, group = cnv, fill = as.character(cnv))) +
  geom_boxplot(outlier.shape = NA) +
  geom_jitter(width = 0.1, size = 0.5) +
  scale_fill_discrete(guide = F) +
  theme_classic()
cnv_protein_example2

# in case you want to save the plot to a pdf file uncomment the following line
#ggsave(filename = file.path(".", "cnv_protein_example2.pdf"), plot = cnv_protein_example2, height = 4, width = 4)
```
...
to make sure we will calculate the CNV buffering for the same set of genes along the cancer types, we will select the genes that have at least 10 samples with protein measures in each cancer type

```{r}
# first set up a function that will calculate the number of samples per cancer type
...
# after nesting the data by gene, we call the select_genes() function we just created for each gene
...
### calculate again the correlations between the CNV and the RNA and the CNV and the protein
```{r}
...
### as before, make a scatterplot with the cor(CNV, RNA) in the x axis and the cor(CNV, Protein) in the y axis
...
# compare the gene attenuation scores between the two cancer types
...
### finally, we can compare the attenuation scores by doing a scatterplot
